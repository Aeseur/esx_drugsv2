print("Meth van, fabriqué par Hugo")
...
AddEventHandler('esx_methcar:start', function()
	...
	if xPlayer.getInventoryItem('acetone').count >= 5 and xPlayer.getInventoryItem('lithium').count >= 2 and xPlayer.getInventoryItem('methlab').count >= 1 then
		if xPlayer.getInventoryItem('meth_pooch').count >= 30 then
				TriggerClientEvent('esx_methcar:notify', _source, "~r~~h~Vous ne pouvez pas contenir plus de méthamphétamine")
		else
		...
		TriggerClientEvent('esx_methcar:notify', _source, "~r~~h~Pas assez de produits pour commencer à cuisiné de la méthamphétamine")
